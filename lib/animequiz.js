class AnimeQuiz {
    ...
    startQuestions() {
        ...
        if (this.waitingForAnswers) {
            return { success: false, message: 'Quiz already in progress' };
        }

        if (this.currentQuestionIndex > 0) {
            return { success: false, message: 'Quiz already started! Current question: ' + this.currentQuestionIndex + '/' + this.totalQuestions };
        }
    ...
    submitAnswer(cid, a, playerName) {
        ...
        if (!this.players.has(cid)) {
            this.players.set(cid, playerName);
            this.scores.set(cid, 0);
        }
