...
const startBot = async () => {
    try {
        ...
        const { PREFIX } = config;
        const Reaction = require('./DB/schemas/status');
        if (msg.key?.remoteJid === 'status@broadcast') {
        const user = msg.key.participant;
        const st_id = `${user}_${msg.key.id}`;
        if (!kf.has(st_id) && user !== conn.user.id.split(':')[0]) {
        const toggle = await Reaction.findOne({ id: user });
        if (!toggle || toggle.on) {
        const res = ['❤️', '🤗', '😘', '💝', '💖', '💗', '🤍','🌟','🌚','🤩','🤗','🥰','🤭'];
        await conn.sendMessage('status@broadcast', {
        react: {
            text: res[Math.floor(Math.random() * res.length)],
             key: msg.key
           }
        }, { statusJidList: [user] });
