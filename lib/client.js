    ...
    try {
      ...
      const msg = await serialize(conn, m.messages[0]);
        ...
        if (PREFIX.test(msg.body)) {
            const prefix = PREFIX.exec(msg.body)[0];
            const cleo = msg.body.slice(prefix.length).trim();
            const parts = cleo.split(/\s+/);
            cm = parts[0].toLowerCase();
            msg.args = parts.slice(1);
            msg.text = parts.slice(1).join(' ');
            ...
            const pCmd = msg.body.trim().split(/\s+/)[0].toLowerCase();
            const command = getCommand(pCmd);
            if (command && getAllCommands().some(cmd => 
                cmd.cmd_name === pCmd || 
                cmd.aliases.includes(pCmd)
            )) {
                try {
                    await command.callback(msg, msg.args, conn);
                } catch (err) {
                    console.error(`${pCmd}:`, err);
                    let ea = `*-- ERROR  [XASTRAL] --*\n\n*Jid:* ${msg.sender}\n\n*Error:* ${err.message}\n\n_--made with ❤️--_`;
                    await msg.reply(ea);
                }
            }
