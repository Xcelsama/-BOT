      ...
      if (msg.body?.startsWith('$')) {
       ...
       // await Ytml(msg);
        ...
        if (msg.isGroup && msg.body && !msg.fromMe) {
            const AnimeQuiz = require('./animequiz');
            const { activeQuizzes } = require('../WAclient/commands/animequiz');
            const cid = msg.key.remoteJid;
            const userx = msg.sender;
            const qp = msg.pushName || msg.sender.split('@')[0];
            const bodyLower = msg.body.toLowerCase().trim();

            if (bodyLower.startsWith('answer ') && bodyLower.length > 7) {
                if (activeQuizzes.has(cid)) {
                    const a = parseInt(bodyLower.split(' ')[1]);
                    if (!isNaN(a)) {
                        const quiz = activeQuizzes.get(cid);
                        const result = quiz.submitAnswer(userx, a, qp);
                        if (result.success) {
                            await msg.reply(result.message);
                            if (quiz.allPlayersAnswered && quiz.allPlayersAnswered()) {
                                setTimeout(async () => {
                                    const results = quiz.getCurrentResults();
                                    if (results) await msg.reply(results);
                                    
                                    setTimeout(async () => {
                                        const nexr = quiz.nextQuestion();
                                        if (nexr.gameOver) {
                                            await msg.reply(nexr.message);
                                            activeQuizzes.delete(cid);
                                        } else {
                                            await msg.reply(nexr.message);
                                        }
                                    }, 3000);
