    ...
    try {
      ...
      const msg = await serialize(conn, m.messages[0]);
        ...
        if (msg.isGroup && msg.body && !msg.fromMe) {
            ...
            try { var SK = await AiChat.findOne({ id: msg.user });
                if (SK && SK.enabled) {
                    ...
                    if (sxt || lid) {
                        ...
                        question = question.replace(/@\d+/g, '').trim();
                            ...
                            const tki = ['generate image', 'create image', 'make image', 'draw', 'generate picture', 'create picture', 'make picture'];
                            const gen = tki.some(keyword => question.toLowerCase().includes(keyword));
                            const ct = ['generate voice', 'text to speech', 'say this', 'speak this', 'voice message', 'convert to voice', 'can you say this'];
                            const isvn = ct.some(keyword => question.toLowerCase().includes(keyword));
                            if (gen) {
                                await msg.react('✨¨');
                                try { let prompt = question;
                                ...
                                await msg.react('✨');
                                try { const response = await msg.pollinations(question, SK.modelIndex || 0);
                                    if (response && typeof response === 'string') {
                                        await msg.reply(response);
                                    } else {
                                        await msg.reply('unailabele baby');
                                    }} catch (error) {
                                    console.error(error);
                                   }
                            }
