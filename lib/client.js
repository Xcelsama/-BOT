...
var {serialize} = require('./Message');
const {connectMongoDB} = require('./DB/database');
const rateOverLimit = new Map();

var sessionDir= path.join("./multi_auth);
if (!fs.existsSync(sessionDir)) {
fs.mkdirSync(sessionDir, { recursive: true });}
const auth = path.join(sessionDir, "creds.json");
if (!fs.existsSync(auth)) {SessionCode(config.SESSION_ID, auth); }
