...
const { getCommand, getAllCommands, getEvents } = require('./command');
    ...
    try {
      ...
      const msg = await serialize(conn, m.messages[0]);
        ...
        if (PREFIX.test(msg.body)) {
            ...
            cm = cleo.split(/\s+/)[0].toLowerCase();
        } else if (msg.body) {
            cm = msg.body.trim().split(/\s+/)[0].toLowerCase();
        } else {
            return;
        }
        const emitter = getEvents('message');
        for (const handler of emitter) {
            try {
                await handler.callback(msg, conn);
            } catch (err) {
                console.error(err);
            }
        }
