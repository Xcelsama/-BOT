...
const connect = async () => {
    ...
    if (message.fromMe || message.sender.split('@')[0] === config.owner ||
    config.mods.includes(message.sender.split('@')[0])) return;
    ...
    const output = result.length > 4000 ? result.slice(0, 3900) : result;
