...
const serialize = async (conn, msg) => {
    if (msg.key) {
        ...
        // Handle lid parameter properly for group messages
        if (msg.isGroup && msg.key.participant) {
            msg.participant = jidNormalizedUser(msg.key.participant);
        }
        
        // Fix lid-related issues in group message handling
    ...
    if (msg.message) {
        ...
        msg.download = async () => {
            ...
            if (msg.type === 'viewOnceMessageV2') {
                messageToDownload = msg.message.viewOnceMessageV2.message[type];
            } else {
                messageToDownload = msg.message[type];
            }
            ...
            try {
                const stream = await downloadContentFromMessage(messageToDownload, type.split('Message')[0]);
                let buffer = Buffer.from([]);
                for await (const chunk of stream) {
                    buffer = Buffer.concat([buffer, chunk]);
                }
