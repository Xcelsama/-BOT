...
const serialize = async (conn, msg) => {
    ...
    if (msg.message) {
       ...
       // msg.text = args.join(' ');
        ...
        let cmd, args;
        if (config.PREFIX.test(msg.body)) {
            const prefix = config.PREFIX.exec(msg.body)[0];
            const args = msg.body.slice(prefix.length).trim().split(/ +/).filter(arg => arg.length > 0);
            const cmd = args.shift()?.toLowerCase();
            msg.args = args;
            msg.prefix = prefix;
        } else {
            const args = msg.body.trim().split(/ +/).filter(arg => arg.length > 0);
            const cmd = args.shift()?.toLowerCase();
            msg.cmd = cmd;
            msg.args = args;
            msg.text = args.join(' ');
