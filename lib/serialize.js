...
const serialize = async (conn, msg) => {
    if (msg.key) {
        ...
        msg.sender = msg.isGroup && msg.key.participant 
    ? jidNormalizedUser(msg.key.participant) 
    : jidNormalizedUser(msg.user);
       const myr = jidNormalizedUser(conn.user.id);
    	const ctx = conn.user.lid ? jidNormalizedUser(conn.user.lid) : null;
        m.fromMe = [myr, ctx].includes(msg.sender);
