...
const serialize = async (conn, msg) => {
    ...
    if (msg.message) {
       ...
       // msg.text = args.join(' ');
        ...
        msg.prefix = config.PREFIX.test(msg.body) ? config.PREFIX.exec(msg.body)[0] : '';
        if (msg.prefix) {
            const msgPrefa = msg.body.slice(msg.prefix.length).trim();
            const parts = msgPrefa.split(/ +/).filter(arg => arg.length > 0);
            msg.cmd = parts[0]?.toLowerCase();
            msg.args = parts.slice(1);
            msg.text = msg.args.join(' ');
        } else {
            msg.cmd = '';
            msg.args = [];
            msg.text = msg.body.trim();
        }
