...
Module({
  ...
  const quotedContent =
    typeof quoted.msg === 'string'
      ? quoted.msg
      : typeof quoted.msg?.text === 'string'
      ? quoted.msg.text
      : typeof quoted.msg?.caption === 'string'
      ? quoted.msg.caption
      : '';

  const lines = quotedContent.split('\n').filter(line => /^\d+\.\s/.test(line.trim()));
  const selected = lines[number - 1];
  if (!selected) return;

  const appName = selected.split('. ')[1];
  if (!appName) return await message.send('Sorry');

  const result = await search(appName, 1);
